...
순수 자바스크립트 차원에서 제공되는 기능들이 추가되었고 훨씬 간결한 코드를 작성할 수 있는 것이 특징이다.
이제는 더 이상 새로운 버전의 자바스크립트 문법인 ES6를 사용하지 않을 이유가 없는 것 같다.
...
ES6 스펙은 그 정도로 많은 기능을 지원하지는 않지만 개발하는데는 충분한것 같다.
...
ES6로 작성하면 `function` 대신 `=>`를 사용할 수 있다.
...
그래서 함수에 넘어오는 파라매터의 갯수가 유동적일 경우 보통 이 arguments 변수를 이용해 파라매터 값을 배열로 만든다.
...
ES6에서는 대신 Rest Parameter를 지원한다.
...
우선 변수하나 선언하는데 코드가 길어지고 자바스크립트 초심자에게는 괴상한 코드다.
...
보통 상수를 정의할 때 언더바(_)로 시작하거나 대문자로 이름 짓는다.
이것은 순전히 개발자가 책임지고 코드이름을 보며 판단해야 한다.
당연히 실수가 있을 수 밖에 없고 버그로 이어진다.

그러나 ES6는 `const`라는 키워드로 상수를 정의할 수 있다.
...
ES6에서는 `class`, `extends` 키워드를 추가했다.
...
단 인터페이스를 지원하지 않는 점은 아쉽다.
아마도 타입이 없는 것과 연관있지 않나 생각한다.
