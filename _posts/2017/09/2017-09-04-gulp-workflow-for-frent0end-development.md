...
이 모든것은 소화하기 힘들지만 도움이 되었으면 좋겠네요. 잊지 말아야 할 것은 `gulpfile.js`가 웹사이트를 빌드하는 코드를 갖고 `package.json`이 특정 웹사이트를 위한 유니크한 것을 포함한다는 것입니다.

이 방법으로 완료하세요. `gulpfile.js`는 거의 변화하거나 추가할 일이 없을 겁니다. 당신이 개발할 사이트의 모든 기본을 위해 사용할 수 있지요. 만약 필요한 특정 행동을 가지고 있다면  추가하고 수정할수 있는 유연함을 가지고 있습니다. 

여기에 전체 `gulpfile.js`가 있습니다. 
