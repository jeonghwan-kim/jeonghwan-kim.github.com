...
이러한 권한에 관리는 라우팅시에 하는데, 권한이 있으면 요청한 화면을 보여주고 그렇지 않으면 적절한 예외처리를 하는 방식이다.
...
이글은 뷰JS로 SPA를 개발할 때 인증 구현 방법에 대해 정리한 내용이다.

![인증](/assets/imgs/2018/03/26/lock.jpg)

## API 준비 

먼저 인증 구현을 위한 3개의 API를 준비하겠다.  Node.JS의 Express 프레임웍으로 구현했다.
아래 저장소를 클론해서 확인해 보자 
...
폴더는 client와 server로 분리했고 이 중 server 폴더가 API를 담당한다.
...
메인 화면 출력할 데이터를 응답하는 API다. 아래와 같은 형태로 응답하는데 
...
인증정보 없이 보냈기 때문에 "Hello world"가 응답된다.

### 2) GET /me

두번째는 인증 후에만 접근할 수 있는 마이 페이지에서 사용할 API다. 
마이페이지에 출력할 데이터를 응답하는데 로그인한 유저 정보이다.
로그인하지 않을 경우 HTTP 상태코드 401 Unauthorized 를 응답한다.
...
401 상태코드와 헤더 정보가 없다는 바디 메세지를 확인 할수 있다. 
...
자동 생성된 코드를 정리한뒤 [vue-router](https://router.vuejs.org/kr/essentials/getting-started.html)로 라우터를 만들고 
...
사이트의 홈페이지를 그릴 Home 컴포넌트부터 만들겠다. 
...
`requireAuth()` 함수를 추가해서 인증 여부에 따라 라우팅을 결정하도록 해보겠다. 
...
현재는 인증되지 않았으므로 로그인 화면으로 이동하게 된다. 
...
![마이 페이지](/assets/imgs/2018/03/26/login.jpg) 
...
때문에 로그인 이후 발생하는 모든 요청에 토큰값을 헤더에 담아 보내면 더 편할 것 같다.
...
스토어에 저장된 토큰정보는 메모리에 있기때문에 브라우져 화면을 갱신함과 동시에 날아가 버린다. 
...
화면을 갱신할때 이 로컬스토리지에 저장된 토큰을 설정하는 로직도 추가한다.
