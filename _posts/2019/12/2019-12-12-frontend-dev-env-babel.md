...
구사하는 언어가 달라 바벨탑이 완성되지 못한 것처럼, 브라우져의 구현의 다양함으로 인해 프론트엔트 코드는 일관적이지 못할 때가 많다.
자바스크립트 표준이 개선되고 브라우져가 버전업 되고 있지만, 여전히 인터넷 익스플로러는 프라미스를 이해하지 못해 개발자로 하여금 적잖은 곤욕을 준다. 
작년까지만 해도 사파리 최신 브라우져는 Promise.prototype.finally 메소드를 사용할 수 없었다.
프론트엔드 개발에서 크로스브라우징 이슈는 초심자에게 무척 혼란스러운 일이다. 
히브리어로 바벨이 '혼돈'이란 뜻인 것처럼 말이다. 

크로스브라우징의 혼란을 해결해 줄 수 있는 것이 바벨이다.
ECMAScript2015+ 스펙으로 작성한 코드를 모든 브라우져에서 동작하도록 호환성을 지켜준다.
...
이렇게 바뀐 코드가 인터넷 익스프로러나 구버전 버전의 브라우져처럼 최신 자바스크립트 코드를 이해하지 못하는 환경에서는 잘 동작하기 때문이다.

바벨을 이용해 아래 코드를 인터넷 익스플로러가 인식하는 코드로 바꿔 보겠다.
...
코드 텍스트를 읽고 추상 구문 트리(AST)로 변환하는 단계를 **"파싱"**이라고 한다. 
이것은 트랜스파일(혹은 빌드)하기에 적합한 자료구조인데 컴파일러 이론에 사용되는 개념이다.
...
그런데...... 이렇게 결과를 보면 빌드 이전과 변한게 하나도 없다.

## 3. 플러그인

기본적으로 바벨은 코드를 받아서 코드를 반환 한다.
...
파싱과 출력만 바벨이 담당하고 변환 작업은 다른 녀석이 처리하데 이것을 **"플러그인"** 이라고 부른다. 
...
이 객체는 바벨의 첫번재 작업인 파싱의 결과물인 추상 구문 트리(AST)에 접근할수 있는 메소드를 제공한다. 
그중 Identifier() 메소드의 동작 원리를 살펴보는 코드다. 

플러그인 사용법을 헬프 문서에서 찾아보자.
...
우리가 하려는것은 ECMASCript2015 로 작성한 코드를 인터넷 익스플로러에서 돌리는 것이다. 
...
이러게 동작하는 것이 [block-scoping](https://babeljs.io/docs/en/babel-plugin-transform-block-scoping) 플러그인이다. 
...
ECMAScript2015+으로 코딩할 때 필요한 플러그인을 일일이 하나씩 설정하는 일은 무척 지난한 일이다.
...
preset-env는 ECMAScript2015+를 변환할때 사용한다.
...
인터넷 익스플로러용 코드로 변환을 위해 env 프리셋을 사용해 보자. 
먼저 패키지를 다운로드한다.
...
기존에 제공했던 프리셋을 사용해 본 경험이 있다면 까다롭고 헷갈리는 설정 때문에 애를 먹었을지도 모르겠다. 
그에 비해 env 프리셋은 무척 단순하고 직관적인 사용법을 제공한다. 

### 5.1 타겟 브라우져 

우리 코드가 Chrome 최신버전(2019년 12월 기준)만 지원하다고 하자. 
그렇다면 인터넷 익스플로러를 위한 코들 변환은 불필요한 작업이 된다. 
...
크롬은 블록스코핑과 화살표 함수를 지원하기 때문에 이러한 결과물을 만들었다.
만약 크롬을 포함한 인터넷 익스플로러도 지원해야 한다면 브라우져 정보만 추가하면 된다.
...
바벨은 다양한 브라우져 환경에서 동작하는 어플리케이션을 일관적인 방식으로 코딩하기 위한 도구다.

바벨의 코어는 파싱과 출력만 담당하고 변환 작업은 플러그인들이 수행한다.
