...
앞서 정리한 몇 가지 기본 패키지를 net/http 패키지를 사용하기 위한 준비 과정이라 생각하자.
...
예제로 구글의 robotx.txt 파일을 요청해서 응답 결과를 확인해겠다.
...
Get으로 요청하여 서버로 부터 응답을 받으면 데이터를 읽은뒤 Close 함수로 응답 바디를 닫아 주어야 한다.
...
그리고 핸들러에서 바디데이터를 디코딩한다. (encoding/json에 대한 설명은 [Go encoding/json 패키지] 참고)
