...
Go 언어를 접하면서 가장 처음 사용하는 것이 fmt 패키지다. 기본적인 출력과 입력 포맷과 관련한 기능을 제공하는데 이 글에서는 fmt에서 제공하는 함수와 타입의 사용법에 대해서 살펴보겠다.

## 템플릿

fmt 패키지는 포맷 템플릿을 이해하는 것이 핵심이다. 이것은 출력할 텍스트와 동사(verb)라고 부르는 변수 출력 형식으로 구성된 문자열이다. 가령 age라는 변수 값을 출력하려면  fmt 패키지의 Printf() 함수를 사용한다.
...
정수형 변수를 출력할 때 사용하는 %d가 바로 동사인데 목적에 따라 다양한 기호를 사용한다.
...
기호를 추가하면 코드 정보까지 출력할수 있다. 아마도 디버깅 용도로 사용하라는 의도 같다.
...
기본적으로 고정된 길이에서 우측 정렬을 따른다. 만약  좌측으로 정렬하려면 - 기호 숫자 앞에 붙인다.
...
Print() 함수는 단순히 문자열을 표준 출력으로 내보내서 터미널에 "HelloWorld"란 문자를 기록한다. 인자가 2개 이상일 경우 순서대로 모두 붙여서 출력한다.

반면 Println()은 여러개 인자를 출력할 때 사이사이 빈 문자를 넣는다. 마지막엔 개행문자까지 추가하기 때문에 가독성이 필요할 경우 Print보다 Prinln을 선호하는 편이다.

Printf()는 앞에서 다루었듯이 문자열과 변수를 함께 출력할때 사용한다. 동사를 포함한 템플릿 문자열과 변수를 인자로 받는다.
...
Print는 표준출력인 os.Stdout를 넘겨 주기 때문에 표준 출력인 터미널 기록하는 원리다.
유닉스 시스템을 표준 입출력을 모두 파일로 처리한다 (/dev/stdin, dev/stdout, /dev/stderr)
...
Fprint() 함수를 직접 사용하면 표준출력 뿐만 아니라 파일에 쓸 수 있다.
...
파일 출력은 앞에 "F"가 붙은 이름의 함수를 제공한다. 유심히 보면 표준출력 함수와 비슷하다.
...
Go에서는 "er"로 끝나는 인터페이스 네이밍 규칙이 있다. Stringer라는 이름의 인터페이스는 문자열 관련한 인터페이스라는 추측을 할 수 있는데 출력 함수의 특정 동사가 이 인터페이스를 사용한다.
...
기본 패키지에 보면 Stringer 인테페이스를 따르는 구조체 들이 있는데 net.IP 와 bytes.Buffer 의 String() 메소드가 그렇다.

Name과 Age 속성을 같는 User 구조체가 Stringer 인터페이스를 충족하도록 해보자.
...
만약 String() 메소드를 구현하지 않고 u를 출력하면 "{Gopher 7}" 결과가 나왔을 것이다. 위 예제에서는 String() 메소드를 구현해서 Stringer 인터페이스를 출족 때문에 "Gopher (7)"결과가 나온 것이다.

이와 유사하게 GoStringer 구조체 인스턴스를 출력할 때 문자열을 반환하도록 하는 인터페이스다.
...
GoString() 메소드 구현전에 "%#v" 동사로 출력하면 "main.User{Name:"Gopher", Age:7}" 결과가 나올 거이다. 구현 후에는 메소드에서 반환한 것처럼 "User: Gopher (7)"로 결과가 나온다.

## 사용자 정의 출력

Stringer나 GoStringer 인터페이스를 구현함으로써 fmt 패키지가 제공하는 출력함수의 결과를 일부 변경할 수 있었다. 출력 형식을 좀 더 자유롭게 제어 할 수 있는데 Formatter와  State 인터페이스가 그 역할을 한다.
...
C를 처음 접할때 마주한 printf(), scanf() 처럼 Go fmt 패키지도 비슷한 함수 네이밍을 사용한다. 새로운 언어임에도 불구하고 다소 친숙하다.
문서에서도 보면 C 스타일의 동사를 사용 했다고 한다. 하지만 좀 더 심플한 방법으로 말이다. (The format 'verbs' are derived from C's but are simpler.)
...
사용자 포맷을 정의하려면 Stringer, GoStringer를 사용한다. Fommter(Scanner) 와 State(ScanState)를 사용하면 훨씬 구체적으로 형식을 제어할 수 있다.
