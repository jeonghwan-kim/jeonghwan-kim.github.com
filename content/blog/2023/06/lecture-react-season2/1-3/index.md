...
이미 만들어둔 작은 컴포넌트를 조합해서 만들었기 때문에 손쉽게 화면 윤곽을 만들 수 있었다. 이것이 바로 역할에 따라 컴포넌트를 분리하고 명시적으로 조합한 효과다.
...
함수 컴포넌트를 클래스 컴포넌트로 바꾸고 order란 상태를 두었다. 렌더 메소드에서는 이 값의 유무에 따라 조건부 렌더링을 할 것이다.
