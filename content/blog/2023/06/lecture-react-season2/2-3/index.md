...
이 중 [pushState](https://developer.mozilla.org/ko/docs/Web/API/History/pushState)는 브라우져의 방문 기록을 브라우져 저장소에 추가하는 함수다. 주소를 바꿀때 이 저장소에 값을 넣으면 방문기록이 하나 생긴다. 브라우져 주소창 값도 변경된다.

Router 주소 변경 핸들러에 다음 코드를 추가해 보자.
...
라우터 컨택스트에 전달할 handleChangePath 함수에 상태 변경 코드 뿐만 아니라 history api를 사용했다. setState가 상태를 변경했다면 pushState는 브라우저에 방문 기록을 추가하는 코드다. 이제 브라우져 주소가 바뀔 것이다.
...
주소는 바뀌었지만 이에 해당하는 화면이 바뀌지 않았다.

pushState 함수를 사용할 때 첫 번째 인자에 빈 문자열만 전달했는데 사실은 state(리액트 스태이트와 무관)를 전달하는 용도다.
...
pushState를 호출할 때 state에 어떤 값을 전달하면 스택에 저장해 둘 수 있다고 한다. 브라우저는 새로운 상태로 이동, 즉 뒤로 가기나 앞으로 가기 버튼을 누를 때마다 이 이벤트를 발생시킨다. 이 때 스택에 저장해둔 state 값을 꺼내서 사용하는 구조이다. 이 이벤트의 이름은 popstate다.

pushState를 호출할 때 이동할 경로 path 를 저장해 두면 좋겠다. popstate 이벤트가 발생할 때 이 값을 꺼내 해당 컴포넌트로 교체할 용도다. 화면을 직접 그리는 것은 아니고 Router의 상태 path를 이 값으로 바꾸면 리액트가 다시 그려줄 것이다.
...
이처럼 어플리케이션은 계층으로 분리하지만 모든 기능을 완벽하게 분리하는 것이 어렵다. 가령 컨테이너에서 주소 정보가 필요한데 페이지에 필요한 데이터를 조회하는 api를 호출하기 때문이다. 페이지 컴포넌트에서도 다른 페이지로 이동하기 위해 주소 정보가 필요하다. 두 계층 모두 주소 정보를 사용하기 때문에 중복 코드를 만들수 있다.
...
로깅 예제를 통해 고차 컴포넌트에 대해 알아보자. 컴포넌트가 얼마나 사용되는지 기록해보자.
...
enhancedProps는 렌더 함수에서 생성한 객체인데 컴포넌트에 추가할 횡단관심사들의 묶음이다. 여기서는 컴포넌트에서 사용할 수 있는 log 함수를 주입했다.
...
헤더와 버튼 코드가 비교적 단순하게 바뀌었다. 로깅 기능이 모두 withLoggin으로 빠졌기 때문이다. 컴포넌트 본연의 기능만 남고 어플리케이션 공통의 기능을 고차 컴포넌트로 옮긴 셈이다.
...
# 2.3.6 네비게이션바 요구사항 구현 match

요구사항 0.3은 현재 화면에 해당하는 링크를 선택된 것으로 표시해야한다. 화면은 주소로 식별할 수 있다. 메뉴 목록화면 /, 장바구니 화면 /cart, 주문 내역 화면 /order. Navbar에서 주소를 알 수 있다면 이것을 식별해 .active 스타일 클래스를 지정해 주면 되겠다.

주소는 Router의 상태 path에 저장되어 있기 때문에 이를 비교해 Navbar 렌더링 로직을 고치자. 컨슈머 컴포넌트를 직접 사용할 수도 있지만 고차 컴포넌트 WithRouter가 주입할수 있도록 하겠다.
...
고차 컴포넌트 안에 match 함수를 추가했다. 비교할 경로를 문자열로 받아 현재 주소와 같은지 비교한다. 이 함수를 타겟 컴포넌트에 주입해서 사용할수 있게 했다. 사용하는 쪽에서는 이 불리언 값에 따라 특정 화면의 주소 접속 여부를 식별할 수 있을 것이다.
...
Navbar를 withRouter로 감싸 WithRouter고차 컴포넌트의 기능을 주입했다. 그 중 위에서 만든 match 함수를 사용했다. 기존에서는 클래스 네임 속성이 고정값이 었다면 이제는 매치 함수 결과에 따라 다르다. 메뉴 목록과 주문 내역 주소가 되면 각 링크가 강조 표시 될 것이다.
...
# 2.3.7 장바구니 요구사항 구현 params

요구사항 2.3은 절반만 해결한 상태다. 상품 정보를 중앙에 표시하기는 하지만 항상 같은 상품만 나오는 상태다. 고정된 상품 아이디를 사용하기 때문이다.

해결하려면 메뉴목록 화면에서 장바구니에 담기 버큰을 클릭할 때 해당 상품 아이디를 장바구니 페이지에 전달해야한다. 장바구니 페이지는 이 상품 아이디를 가지고 상품 가져오기 api 를 호출해야 한다.

페이지 간에 데이터를 전달하는 방법은 주소를 이용하는 방법이 있다. 주소창에 쿼리 스트링으로 전달하면되겠다. /cart?prodcutId={상품아이디값}. 주문하기 버튼의 동작을 수정하자.
...
CartPage에게 라우터 기능을 주입했다. 주소를 통해 전달된 상품 아이디가 있을 경우에 데이터를 불러 오도록 했다. 이전에는 장바구니 화면에서 고정된 상품만을 보여줐지만 이제는 주소의 쿼리 문자열에 따라서 상품 정보를 렌더링할 것이다.
...
이전 장에 이어서 라우터에 대해 학습했다.

- 히스토리 api. 주소 상태를 관리하는 pushstat를 사용했다. 주소 이동시 발생하는 이벤트 popstate를 감지했다.
- 고차 컴포넌트. 어플리케이션 횡단 관심사를 해결하는 패턴이다. 라우팅 기능을 계층을 아우르는 공통 관심사로 분리하고 이를 제공하기 위한 WithRouter 컴포넌트를 만드는 withRouter 함수를 만들었다.

수업에서는 만든 라우터와 비슷한 역할의 리액트 라우터 라이브러리도 참고하면 좋다.
...
- [리액트 고차컴포넌트 | 김정환 블로그](https://jeonghwan-kim.github.io/2022/05/28/react-high-order-component)
