...
알 수 없는 스크립트가 실행되어 위험

- 쿠키 등 중요한 데이터를 탈취할 수 있다.
- 자바스크립트로 DOM을 수정해 화면을 조작할 수 있다.

데이터를 안전하게 처리해 예방할 수 있다.

- 세니타이즈(Sanitize): 허용하지 않은 문자를 제거한다.
- 이스케이프(Escape): 허용하지 않은 문자를 안전한 문자로 대체한다.
- 라이브러리: [DOMPurify](https://github.com/cure53/DOMPurify), [he](https://github.com/mathiasbynens/he)
