...
TanStack Query는 서버 상태를 클라이언트에서 효율적으로 다루는 도구다. 처음 접할 때는 useQuery() 정도만 사용해도 충분해 보였고, 실제로 기본 기능만으로도 많은 일을 할 수 있었다.

실무에서 다양한 API 유청을 다루고 보니 queryKey 설계, 오류 처리, 쿼리 무효화, 로딩 UI 등 고려해야 할 요소들이 훨씬 많았다. 이 때 내부 원리와 세부 기능들을 이해하고 활용하면, 더 유연하고 안정적인 코드를 작성할 수 있을 것이다.

TanStack Query를 실무에 적용하면서 경험한 시행착오와 활용 팁을 정리해 보겠다. 공식 문서에 명시돼 있지만 자주 간과되는 부분이나, 문서에는 없지만 실무에 꼭 필요한 내용을 중심으로 구성했다.
...
모든 Query의 오류를 공통으로 처리하려면, QueryClient 인스턴스 생성 시 onError 콜백을 지정한다.
...
조건부 쿼리를 실행을 위한 옵션이다. 이 옵션 설정한 조건에 따라 queryFn가 동적으로 실행될 것이다.
...
주의해야할 것이 있다. 이 옵션은 마운트 시점, 그러니깐 초기에만 동작한다는 특징이 있다. 한 번 마운트되고 나면, enabled가 false -> true, 또는 ture -> false로 바뀌더라도 queryKey와 캐시 전략에 따라 호출된다. 마운트 이후에는 enabled 옵션이 무시된다는 말이다.

언제든지 쿼리를 특정 조건에 따라 동적으로 실행하려면 refecth를 사용하는 방법이 있다.
...
TanStack Query는 단순한 데이터 패칭 도구를 넘어, 서버 상태를 체계적으로 관리할 수 있게 해주는 실무 필수 도구다. 핵심 동작원리와 세부 옵션을 이해하면 복잡한 요구사항도 깔끔하게 해결할 수 있다. 정리한 몇 가지 팁이 실무에서 TanStack Query를 더 깊이 있게 활용하는 데 도움이 되었으면 좋겠다.
