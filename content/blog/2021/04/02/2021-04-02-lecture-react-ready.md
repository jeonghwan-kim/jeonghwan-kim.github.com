...
무언가를 공부하려면 이론과 실습이라는 두 가지 방법을 사용한다. 가령 기타를 배운다고 생각해 보자. 음악 이론부터 시작해 악보 보는 법, 악기 소개 등을 먼저 공부한다. 그리고나서 기타 지판을 손가락으로 짚고 줄을 튕기며 이론을 몸으로 익히는 실습 과정을 통해 머리와 몸으로 기타를 배운다.

한편 이론과 실습의 순서를 반대로 하는 경우도 있다. 유투브에서 누군가 멋지게 연주하는 모습을 보고 감탄한다. 기타에 대해 모르지만 하나씩 하나씩 떠듬 떠듬 연주를 따라해 본다. 처음에는 말도 안되게 어렵겠지만 여러 번 반복하면 제법 비슷하게 소리낼 수 있는 수준에 이르게 된다. 기타 연주에 재미를 붙인 그는 다른 곡들도 그런 방법으로 공부한다. 그리고나서 이론을 접하면 ‘아! 이게 바로 이런 원리구나' 라는 깨달음을 얻을 때가 있다.

> 백문이 불여일타

나는 기타를 이런 방법으로 공부했는데 프로그래밍도 후자의 경우가 많았던 것 같다. 문서에 나온 헬로월드 코드나 책에 나온 예제 코드를 키보드로 입력해 결과물을 먼저 만들어 본다. 그리고 나서 머릿속에 구조를 추측해 본 뒤 문서와 코드를 읽으며 이론을 학습한다.

이론을 먼저 공부한 적도 있었는데 비교적 재미있지는 않았다. 효과가 좋지 않다는 말이 아니다. ‘재미’란 말이다. 프로그래밍과 재미란 단어가 마치 타고난 해커들한테나 어울리는 것처럼 보인다. 하지만 그렇지 않은 사람들에게 프로그래밍은 어려운 외국어이거나 더 어럽게 보인다. 여기에 재미 요소마저 없다면 프로그래밍은 어려운 것, 똑똑한 사람들만 하는 것이라고 치부해 버리지 않을까?
...
리액트 입문자를 위한 컨텐츠를 구상하던 차에 이러한 접근법을 선택했다. 결과물 구현에 초점을 두면서 중간에 이론을 소개하는 방식의 연재물이다. 나중에는 스스로 이론을 한 번 체계화하는 과정이 반드시 필요한데 이것은 이 연재물을 읽고 난 뒤 독자의 몫으로 남겨 두겠다.
...
추천 검색어의 키워드를 선택하면 검색 결과로 이동한다. 최근 검색어 탭을 선택하면 추천 검색어 목록이 사라지고 최근 검색어 목록으로 교체된다. 최근 검색어는 검색 일자가 표시되고 삭제할 수 있는 버튼이 우측에 위치한다. 검색어 목록 중 하나를 클릭하면 검색 결과로 이동한다.

정리하면 검색어를 입력하고 결과를 확인할 수 있는 웹 페이지를 만들 것이다. 사용자가 쉽게 검색할 수 있도록 추천 검색어와 최근에 입력했던 검색어를 제공하는 기능도 있다. 아무쪼록 결과물을 보고 흥미가 생겼으면 한다. 이제 프로젝트를 시작하기 위해 실습 환경을 구성해 보도록 하자.

## 실습 환경 구성

결과물을 미리 확인했으니 이걸 만들기 위한 개발 환경을 만들겠다. 실습에서는 세 가지 도구를 사용한다.

- Node.js (필수)
- VS Code
- 크롬 브라우저

Node.js로 프론트엔드 개발 환경을 구성하는 것은 이제 무척 자연스럽다. react.js 따위의 라이브러리를 설치하거나 개발 도구를 실행하기 위한 목적으로 Node.js를 사용한다. 자세한 내용은 아래 글을 참고하자.
...
항상 두 가지 버번을 제공하는데 왼쪽이 장기적으로 지원하는 안정적인 버전이다. 이 설치파일을 받아서 설치하면 된다. 반면 왼쪽은 최신 기능인데 왼쪽이 짝수라면 오른쪽은 항상 홀수 버전을 사용한다.

에디터는 주로 VS Code를 사용하는 편이다. 실습 뿐만아니라 실무에서도 사용할만큼 훌륭한 무료 개발 도구다. 반드시 이 에디터를 사용해야하는 것은 아니다. 손에 익숙한 것을 사용해도 무방하다. VS Code를 사용하려면 사용하는 운영 체제별로 설치 파일을 다운받아 설치할 수 있다.

- https://code.visualstudio.com/

브라우져는 크롬을 사용한다. 나는 가끔 파이어폭스를 사용하기도 하는데 이것도 각자 취향에 맞게 선택해서 사용해도 무방하다. 단 ES6 모듈 시스템을 지원하는 브라우져인지 확인하자. 최신 버전의 브라우져는 인터넷 익스플로러를 제외하곤 대부분 가능하다.

- https://caniuse.com/?search=module

## ⭐ 중간 정리

키보드 자판을 두드리며 몸으로 익히는 학습 방법이 재밌었다. 리액트 학습의 러닝 커브를 낮추어 보고자 실습 위주의 내용을 준비했다. 물론 이론도 적당히 짚어가면서 리액트의 핵심 내용을 하나씩 파악해 보자.

실습 결과물을 미리 살펴봤는데 간단한 검색 화면을 개발할 예정이다. Node.js, VS code, 크롬 브라우저로 실습 환경을 구성했다.

실습 환경을 갖추었다고 바로 코딩을 하긴엔 우리가 아는게 전혀 없다. 소개팅을 할 때도 첫 만남 전에 주선자로부터 간단한 소개를 받고 설레는 마음으로 약속 날짜를 기다린다. 잠시 리액트 소개팅 주선자 역할을 해 보겠다.

# 순수 자바스크립트 1

리액트를 소개시켜 달라니깐 왜 갑자기 순수 자바스크립트냐고?

비교와 대조. 무엇을 이해할 때 반대쪽에 있는 개념과 비교하면서 이해력을 높이는 방법이다. 가령 아이패드 홈페이지가 그렇다. 기본형, 프로, 에어, 미니 여러가지 모델이 있는데 각자 설명 화면에 들어가도 뭘 사야할지 금방 눈에 들어오지 않는다. 애플은 각 모델을 비교할 수 있는 화면도 제공하는데 난 이 화면을 좋아한다. 쉽게 비교할 수 있기 때문에 내게 맞는 모델을 쉽게 찾을 수 있기 때문이다.

만약 리액트 같은 UI 라이브러리가 없으면 어떻게 개발할까? 자바스크립트와 브라우저의 돔 스크립트만을 이용(일명 순수 자바스크립트, 바닐라 자바스크립트)해서 개발할 수 있다. 결국에 라이브러리도 이걸로 구현했기 때문이다. 같은 결과물을 순수 자바스크립트 버전과 리액트 버전으로 만들어 봄으로써 리액트의 역할과 이점을 더 확실하게 이해할 수 있을 것이다.

리액트에 앞서 순수 자바스크립트로 먼저 검색화면을 만들어볼 작정이다. 이 장은 리액트 학습을 위해 의도를 가지고 만든 코드라고 봐 주면 좋겠다.

## MVC

고전적인 문제를 해결하는데는 그 동안 여러번 해결해 왔던 다양한 방식이 존재하고 이것을 "패턴"이라고 부른다. 프로그래밍도 문제를 해결하는 여러 가지 접근법이 있고 이것을 "디자인 패턴"이라고 한다. 화면 개발에서 많이 쓰이는 것 중의 하나가 **MVC 패턴**이다.

모델(Model), 뷰(View), 컨트롤러(Controller)라는 세 개의 계층으로 구성되고 각자의 역할을 가지고 서로 협력해서 문제를 해결한다.

**모델(Model)**은 데이터를 관리하는 역할을 한다. 가령 API나 브라우저 로컬 저장소에 있는 데이터를 가져와 어플리케이션에서 사용할 수 있는 모양으로 만든다. 데이터를 수정하거나 삭제할 수 있는 방법도 알고 있다.

**뷰(View)**는 사용자가 볼 수 있는 화면을 관리하는 역할을 한다. 데이터를 돔에 출력하거나 사용자가 발생한 이벤트를 처리하는 기능을 수행한다. HTML, CSS도 뷰로 분류할 수 있다.

어플리케이션이 동작하려면 모델이 가지고 있는 데이터를 뷰에게 전달해 화면에 출력해야 한다. 반대로 뷰에서 발생한 이벤트에 따라 모델은 데이터를 관리해서 어플리케이션 상태를 관리해야한다. 역할에 따라 서로 격리되어 있는 모델과 뷰를 연결하고 움직이는 주체가 바로 **컨트롤러(Controller)**다.
...
생성 시점에 데이터 저장소 객체를 받아서 내부에 관리한다(1). 생성자 인자로 들어온 스토리지(storage)는 어플리케이션에서 사용할 도메인 데이터 저장소인데 여기서는 검색을 위한 상품 목록이나 추천 검색어, 그리고 사용자가 과거에 입력했던 검색어 목록이 담겨있을 것이다.

스토어는 이러한 스토리지 데이터를 어플리케이션에서 사용할 수 있도록 변환하는 중간 계층 역할을 한다. 조회, 수정, 추가 등 일련의 데이터 처리를 이 스토어가 담당하게 된다.
...
화면 출력을 위한 돔 엘리먼트를 생성 시점에 받아서 저장한다(1). 뷰를 화면에 보이고 숨기기 위해 스타일의 display 속성을 사용할 것인데 이것을 위해 원래 값을 저장해 둔다(2). hide()와 show() 메서드에서 이 값을 활용한다(3, 4).

엘리먼트 메소드의 addEventListener()와 dispatchEvent() 함수를 래핑한 on(), emit() 유틸리티 함수를 미리 만들어 두었는데 같은 이름으로 View 클래스 메소드를 추가해 두었다(5, 6). 뷰가 관리하는 엘리먼트에서 발생한 이벤트의 핸들러를 연결하거나 외부에서 구독하기 위한 용도다.

모델과 다음에 소개할 컨트롤러와 달리 뷰는 여러 개를 만들 것이다. 검색폼 뷰, 검색결과 뷰 등. 따라서 공통의 로직을 View 클래스에 정의하고 각자 이를 상속하도록 할 것이다.
...
스타일은 미리 준비해둔 클래스 이름을 사용한다. 검색어를 입력할 수 있는 인풋 요소와 입력한 내용을 제가할 삭제 버튼이 보일 것이다.
...
부모인 View 클래스의 생성자 함수를 호출해 돔을 내부 변수로 저장한다(1). qs()는 docuemnt.querySelector()를 호출하는 헬퍼 함수다.

인풋요소와 리셋 요소를 제어하기 위해서 inputElement와 resetElement라는 변수도 저장한다(2, 3).
...
지금은 컨트롤러가 폼뷰를 사용하지는 않지만 앞으로 사용하기 위해 저장해 두었다. 앞으로 생성되는 모든 뷰는 이런식으로 메인에서 객체를 생성만든 후 컨트롤러 생성자 인자로 전달하고 컨트롤러는 내부에 이 폼을 저장하는 일련의 순서를 따를 것이다.

~~💡 요구사항: 검색 상품명 입력 폼이 위치한다.~~

## 검색폼 2

**💡요구사항: 검색어를 입력하면 x버튼이 보이고 없으면 x 버튼을 숨긴다**
...
검색어를 입력했다는 것을 알려면 이벤트가 발생했다는 것이다. 뷰에서 발생하는 모든 이벤트를 처리기를 연결하기 위해 bindEvents() 메서드를 호출한다(1). 미리 저장해 둔 inputElement 돔에서 keyup 이벤트가 발생하면 handleKeypu() 메서드를 호출하는 함수를 연결했다(2). on은 addEventListener() 함수를 호출하는 헬퍼 함수다. handleKeyup() 메소드에서는 입력한 문자열의 길이에 따라 버튼의 노출 여부를 결정한다. 입력한 문자가 있으면 x 버튼을 표시하고 그렇지 않으면 숨기도록 요구사항을 구현했다(3).
...
지금은 입력 폼을 다루는 FormView의 역할 정의하는 중인데 검색 결과는 이 녀석의 역할이 아닌것 같은가? 그렇다면 제대로 생각하고 있는 것이다. 나중에 만들 ResultView가 더 어울리다.
...
subscribViewEvents() 메소드는 각 뷰에서 발생하는 이벤트를 구독하는 메서드다. 폼 뷰에서 입력한 검색어와 함께 발행되는 이벤트 "@submit"을 검색 메소드가 처리하도록 연결했다(2). search() 메소드에서는 입력한 키워드를 인자로 받아서 간단히 로그만 찍었다(3).

실제 검색 결과를 출력하는건 검색폼 요구사항을 모두 구현한 뒤에 ResultView에서 다룰것이다. FormView와 컨트롤러의 역할은 여기까지다.
...
handleKeyup() 메소드에서되 검색어가 삭제되었다는 것은 입력한 문자의 길이로 알 수 있다(3). 이걸 계산해서 "@reset" 이벤트를 발행한다(3).
...
부모인 View 클래스의 show() 메서드를 오버라이드 한다. 검색 결과 데이터를 배열 인자로 받아 길이에 따라서 출력하는데 데이터가 있을 경우와 없을 경우로 구분해 UI를 만든다(2). 그리고나서 부모 클래스 View의 show() 메서드를 호출해 화면에 노출한다(3).

생성자 함수를 다시보면 Template 객체를 만들어 내부 변수로 저장했다(4). show() 메서드에서 사용하는 이 객체는 뷰에서 사용하는데 필요한 마크업 문자열을 만드는 역할을 한다. 템플릿 클래스를 이어서 보자.
...
FormView에서 "@submit" 이벤트가 발생하면 컨트롤러의 search() 메소드가 호출되고 검색어가 인자로 전달되는데 스토어의 search() 메소드를 호출해서 이를 저장해 둔다(2). 이어 변경된 상태에 따라 화면을 그리도록 render() 메소드를 호출한다(3).

render() 함수에서는 검색어가 있을 경우에 검색 결과를 노출하고(5) 그렇지 않을 경우 숨기도록 했다(5). 컨트롤러의 render() 메소드는 여러 뷰들에게 노출 여부를 지시해 전체 화면을 그리는 역할을 한다. 지금은 우선 ResultView만 사용했는데 이후 뷰가 추가될 때마다 render() 메소드에서 제어할 것이다.
...
검색폼 초기화는 이미 FormView에서 처리하고 있기 때문에 검색 결과를 숨기는 기능을 구현하면 된다. 폼뷰의 "@reset" 이벤트를 받아 처리하는 reset() 메소드에 로직을 추가하면 되겠다.
...
세부적으로 정의된 요구사항을 하나씩 보며 사용자 입력을 위한 검색폼, 검색 결과를 구현했다. MVC의 등장 인물인 모델, 뷰,컨트롤러의 역할을 소개하고 어떻게 협력하는지 코드를 통해서 이해했다.

뷰는 눈에 보이는 UI를 담당한다. 돔을 조작해 실제로 화면을 그린다. 그리고 사용자로부터 입력을 받고 적절히 처리한 뒤 자신의 역할이 밖의 것은 외부로 전달하기 위해 커스텀 이벤트를 발행한다.
...
이어서 다음장도 나머지 기능을 순수 자바스크립트로 구현해 보자.
...
부모 클래스인 View에 정의된 show() 메더드를 재정의한다. 화면에 노출하기 전에 템플릿 객체로부터 탭 리스트 마크업 문자열을 가져와 돔에 추가하기 위해서다(3). 이어서 show() 메서드를 호출하면 템플릿에서 가져오 돔이 화면에 보일 것이다.
...
탭 목록을 출력하기 위한 HTML 문자열을 반환하는 것이 getTabList() 메소드다. 탭 키를 배열로 만들고 (3) key, label 로 구성된 객체로 변환한 뒤(4) \_getTab() 메소드로 탭을 위한 마크업 문자를 생성한다(5).
...
CSS 클래스 "active"를 탭 엘리먼트에 추가하면 민트색으로 강조되면서 선택된 UI로 표시할 수 있다. 클래스는 미리 준비해 두었다.
...
탭뷰를 노출하는 show() 메서드에 선택된 표시할 탭 정보를 인자로 받도록 수정한다(1). li 엘리먼트 중 선택한 탭 엘리먼트를 찾는데 data-tab 속성값과 인자의 tab 문자열 값을 비교해 일치하는 녀석에서 active 클래스 이름을 붙여 준다(2).
...
render() 함수 내용 중 탭뷰를 보여주는 부분에 스토어에서 가지고 있는 선택된 탭을 탭뷰 측으로 전달했다. 탭뷰는 이 정보를 가지고 선택된 탭을 표시할 것이다.
...
생성자에서 엘리먼트를 저장하고 템플릿 객체를 생성하는 코드는 비슷하다(1). 키워드 데이터에 따라 동적으로 출력하기 때문에 show() 메서드를 오버라이드한다(2). 배열을 받도록 시그니처를 변경했다. 데이터가 있으면 템플릿을 통해 리스트 마크업 문자열을 가져오고 그렇지 않으면 빈 메세지를 가져온다. 이어서 부모의 show() 메소드 호출도 잊지 않아야 한다.
...
KeywordView도 main() 함수에서 객체로 만들고 컨트롤러에 전달되어 내부 변수로 저장되어야 한다.
...
KeywordView에서 보낸 "@click" 이벤트를 구독한다. 이벤트에서 키워드 값을 뽑아 search() 함수로 전달한다(1). FormView에서 발생한 '@submit' 이벤를 처리하는 것과 비슷하다.
...
탭, 추천 검색어, 최근 검색어에 대한 요구사항을 모두 구현했다. 요구사항은 다르지만 모델, 뷰, 컨트롤러의 역할을 잘 생각해서 일감을 분담하면 그렇게 어렵지만은 안게 요구사항을 구현할수 있었을 것이다.

사실 좀 반복같기도 하다. 외부 사정을 모르는 뷰와 모델을 먼저 만들었다. 뷰는 외부에서 데이터가 들어온다고 가정하고 화면을 그린다. 그리고 자신의 역할이 아니라고 판단되면 커스텀 이벤트로 외부에 이를 알린다. 모델도 데이터만 관리한다. 어플리케이션 데이터와 화면 출력을 위한 상태를 관리한다.

이 둘을 사용해서 어플리케이션을 돌아가게 하는것이 컨트롤러의 역할이다. 데이터를 변경하고 이를 뷰에 반영해서 화면을 렌더링한다. 반대로 뷰에서 사용자 입력이 발생하면 이를 모델에 저장해서 어플리케이션 상태를 관리한다.

# 🌟 최종정리

백문이 불여일타. 여러번 듣는 것 보다 한번 손으로 코딩해보는 것이 더 효과적으로 학습할수 있다는 필자의 경험을 바탕으로 시리지를 구성했다. 쇼핑몰 검색페이지라는 결과물을 스샷 위주로 확인했다. 실습 위주이다 보니 실습 환경을 먼저 구성했다.

리액트를 접하기 앞서 순수 자바스크립트를 살짝 맛보았다. 그냥 구현하기 보다는 문제해결을 위한 대표적인 패턴 중 하나인 MVC 패턴으로 결과물을 구현했다. UI 라이브러리의 필요성과 리액트의 장점을 강조하려는 의도다.

이제 리액트를 학습할 준비을 마쳤다. 더 이상 가다가는 이게 리액트인지 순순자바스크립트를 말하려는지 헷갈리겠다. 어서 리액트로 넘어가자.
